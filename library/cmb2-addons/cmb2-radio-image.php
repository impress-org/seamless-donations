...
License: GPL-2.0+
*/
///////// MODIFIED BY DG -- ADDED SECOND CLASS: CMB2_Radio_Static to show images
///                         but appear to be radio buttons.
...
if( !class_exists( 'CMB2_Radio_Static' ) ) {
    /**
     * Class CMB2_Radio_Image
     */
    class CMB2_Radio_Static {

        public function __construct() {
            add_action( 'cmb2_render_radio_static', array( $this, 'callback' ), 10, 5 );
            add_filter( 'cmb2_list_input_attributes', array( $this, 'attributes' ), 10, 4 );
            add_action( 'admin_head', array( $this, 'admin_head' ) );
        }
        ...
        public function attributes($args, $defaults, $field, $cmb) {
            if ($field->args['type'] == 'radio_static' && isset($field->args['images'])) {
                foreach ($field->args['images'] as $field_id => $image) {
                    if ($field_id == $args['value']) {
                        $image = trailingslashit($field->args['images_path']) . $image;
                        $args['label'] = '<img src="' . $image . '" alt="' . $args['value'] . '" title="' . $args['label'] . '" />';
        ...
        public function admin_head() {
            ?>
            <style>
                .cmb-type-radio-static .cmb2-radio-list {
                    display: block;
                    clear: both;
                    overflow: hidden;
                }

                .cmb-type-radio-static .cmb2-radio-list input[type="radio"] {
                    display: none;
                }

                .cmb-type-radio-static .cmb2-radio-list li {
                    display: inline-block;
                    margin-bottom: 0;
                }

                .cmb-type-radio-static .cmb2-radio-list input[type="radio"] + label {
                    /*border: 3px solid green;*/ /* Deleting makes entire image field appear static */
                    display: block;
                }

                .cmb-type-radio-static .cmb2-radio-list input[type="radio"] + label:hover,
                .cmb-type-radio-static .cmb2-radio-list input[type="radio"]:checked + label {
                    border-color:white ;
                }

                .cmb-type-radio-static .cmb2-radio-list li label img {
                    display: block;
                }
            </style>
            <?php
    ...
    $cmb2_radio_image = new CMB2_Radio_Static();
